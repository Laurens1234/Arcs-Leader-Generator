...
def create_guild_card(input_data):
    ...
    # Prefer the small background image for all guild cards. If absent,
    # fall back to the larger background with a configurable random crop.
    ...
    if not input_data.get("use_large_bg", False) and os.path.exists(small_bg_path):
        ...
        # Use themed star background zoomed into a random region (fallback to black)
...
def main(argv):
    ...
    parser.add_argument(
        ...
        dest="yaml_file",
        default=None,
        help="Optional path to a YAML file containing guild cards (loads list).",
    )
    ...
    # Allow loading guild cards directly from a YAML file provided on the CLI.
    if args.yaml_file:
        try:
        ...
        if loaded is None:
            print(f"Error: YAML at {args.yaml_file} is empty or invalid")
            return 3
        ...
        cards_source = loaded
        print(f"[guildCards] Loaded {len(cards_source)} entries from {args.yaml_file}")
